module vd_gas_zhang
    ...
    subroutine Gas_Zhang(z2,zl,z0_f,ustar,t2,ts,srad,rh,fcld,pp,coszen,mlu,snow,  &
                            io3,inh3,conc_nh3,henry,henso2,f0,diffrat,rscale,lat_f,vd,fnet)
    ...
    !    rscale    Rs scaling for extremely soluble gases
    !    lat_f     leaf area index                          (1)

    !Output arguments:
    !    vd   deposition velocity                           (m/s)
    !    fnet     net flux of NH3 exchange                  (ug/m^2-s)
    ...
    !Step 1. Calculate needed inputs variables
    !!Step 1-1. Calculate snow depth (unit = cm)
        sd          = snow*1000.                          !*100 for m->cm, *10 for SWE->depth

    !!Step 1-2. Calcualte precipitation rate (unit = mm/hr)
        volrat      = pp/rhoh2o
        prec        = (vlorat/1.0e-7)**1.27

    !!Step 1-3. Calculate deposition parameters for species
        alpha       = henry/henso2
        beta        = f0
        rm          = 1./(henry/3000/ + 100.*f0)          !rm (mesophyll resistance) follow Wesely et al.,(1989) eqn (6)
        rm          = amax1(rmin, rm)
    ...
    !Define function for saturation vaport pressure (mb)
    ...
    !Step 3. Calculate stomatal resistance for water vapor only,Rst 
    ...
    !Step 3-1. Calcualte direct and diffuse PAR from solar radiation and solar zenith angle    
        ...
        if (srad >= 0.1 .and. ts < (tmax(i) + 273.15) .and. ts > (tmin(i) + 273.15) .and. lai_f >0.001 .and. coszen > 0.001) then
            ...
            pardif = sv - parfir                                         ! Par from diffuse radiation            
     ...
     !Step 3-3. Stomatal conductance before including effects of temperature, vapor pressure deficit, and water stress.
         ...
         !function for temparature effect
         t = ts -273.15
         ...
         d0 = es(ts)*(1.-rh)/10     !unit = kap
         gd = 1.- bvpd(i)*d0

         !functiion for water stress effect
      ...
      !Step 4x. Calculate fraction of stomatal blocking under wet conditions
      !!Step 4x-1. Decide if dew or rain occurs
      if (fcld < 0.25) then
          coedew = 0.3
      ...
      dq = 0.622/1000.*es(ts)*(1.-rh)*100      !unit g/kg
      dq = amax1(0.0001,dq)
      usmin = 1.5/dq*coedew
      ...
      !!Step 4x-2.Decide fraction of stomatal blocking due to wet conditions
      Wst = 0
    ...
    !Step 4. Calculate in-canopy aerodynamic resistance, Rac
        Racz = Rac1(i) + (lai_f - lai_ref(i,14))/(lai_ref(i,15)-lai_ref(i,14)+1.e10)*(rac2(i)-rac1(i))    !Zhang et al., (2003) eqn (7a)
        Racz = Racz*lai_f**0.25/(ustar*ustar)                                        !Zhang et al.,(2003) eqn (7)

    !Step 5. Calculate ground resistace,Rg
    !!Step 5-1. Calcualte ground resistance for O3
    ...
    !!Step 5-2. Calcualte ground resistance for SO2
        if (i ==2) then
    ...
    !Step 6. Calculate cuticle resistance,Rcut for o3 and so2
        if (rcutdo(i) <= -1) then
            rcuto_f = 1.e25
            rcuts_f = 1.e25
            ...
            rcuto_f = ractwo(i)/(lai_f**0.5*ustar)       !Zhang et al.,(2003) eqn (9b)
     ...
     !Step 5&6 supplemental.if snow occurs, Rg and Rcut are adjusted by snow cover fraction
        fsnow = sd/sdmax(i)
      ...
      !Step 7. Calculate quasi-lamina resistance, Rb
         vi = 145.8 * 1.e-4*(ts*0.5 + t2 *0.5)**1.5/(ts*0.5 + t2*0.5 +110.4)    !diffusivit for each gas species,source unclear
         rb = 5.*(v1/di)**0.666667/ustar

      !Step 8. Calculate stamatal resistance for each species from the ratio of diffusivity of water vapor to the gas species
         dvh2o = 0.001*ts**1.75 * sqrt((29.+18.)/29./18.)
         dvh2o = dvh2o/(dair**0.3333 + dh2o **0.3333)**2
         rs = rst*dvh2o + rm
      !Step 9. rescale cuticle and ground resistances for each species
         rcut = 1./(alpha/rcuts_f + beta/rcuto_f)
         rg = 1./(alpha/rgs_f + beta/rgo_f)

      !Step 10. bi-directional NH3 flux calculation
          if (inh3 == 1) then
              ...
              if (i >= 13 .and. i <= 20) then
                  ...
                  f_net = -(c_nh3 _d_c) * rabi           !net flux (ug/m^2-s), positive flux --> emissions
      ...
      !Step 11. Calculate total surface resistance
       ...
       !Step 12. calculate dry deposition velocity
           vd = 1./(ra+rb+rc)
